...
public class Listeners implements Listener {
    ...
    public void projectileLand(ProjectileHitEvent e) {
        if(e.getEntity() instanceof Trident && e.getEntity().getPersistentDataContainer().has(new NamespacedKey(BoxPlugin.instance, "HEAL_SPEAR"), PersistentDataType.INTEGER)) {
            ...
            List<LivingEntity> near20 = e.getEntity().getNearbyEntities(15, 3, 15).stream().filter(entity -> entity instanceof Damageable && entity.getLocation().distance(location) <= 10).map(entity -> (LivingEntity) entity).collect(Collectors.toList());
